...
class Browser_list_fieldSwitchController extends ControllerSwitchClass {
	...
	public function index()
	{
		...
		I18n::load_lang('forms', 'common');
		
		Utils::load_libraries(array('login', 'generate_admin_ng', 'simplelist', 'forms/selectmodelformbyorder', 'forms/selectmodelform'));
		...
		$module=@Utils::slugify($_GET['module']);
		$model_name=@Utils::slugify($_GET['model']);
		$field_ident=@Utils::slugify($_GET['field']);
		$field_fill=@Utils::slugify($_GET['field_fill']);
		$category_model=@Utils::slugify($_GET['category_model']);
		$category_model_field=@Utils::slugify($_GET['category_model_field']);
		$category_field=@Utils::slugify($_GET['category_field']);
		$field_parent_category=@Utils::slugify($_GET['field_parent_category']);
		...
		if($yes_go==0)
		{
			...
			if(isset(PhangoVar::$model[$model_name]) && isset(PhangoVar::$model[$model_name]->components[$field_ident]))
			{
				...
				if($category_model!='' && isset(PhangoVar::$model[$category_model]) && isset(PhangoVar::$model[$category_model]->components[$category_model_field]) && isset(PhangoVar::$model[$model_name]->components[$category_field]) )
				{
					...
					{
						echo load_view(array('title' => I18n::lang('common', 'filter_by_category', 'Filter by category'), SelectModelFormByOrder('category_id', '', $_GET['category_id'], $model_name, $category_model, $field_parent_category, $where='', $null_yes=1) ), 'content');
					}
					else
					{*/
						
					
					$form_html='<form method="get" action="'.$url_options.'/">'.SelectModelForm('category_id', '', $_GET['category_id'], $category_model, $category_model_field, '').'<input type="submit" value="'.I18n::lang('common', 'send', 'Send').'"/></form>';
					
					echo load_view(array('title' => I18n::lang('common', 'filter_by_category', 'Filter by category'),  $form_html), 'content');
			...
			$title=''; //I18n::lang('forms', 'search_on_table', 'Search on table');
...
function ChooseOptionsListModel($url_options, $model_name, $id, $arr_row)
{
	...
	$arr_options[]='<a href="#" class="select_id select_id_'.$id.'"><span style="display:none;" id="text_field_'.$id.'">'.$arr_row[$field_ident].'</span>'.I18n::lang('common', 'select', 'select').'</a>';
